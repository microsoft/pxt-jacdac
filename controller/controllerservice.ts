namespace jacdac {
    ...
    export class ControllerService extends Broadcast {
        ...
        private connectClient(
            ...
            for (let i = 1; i < this.players.length; ++i)
                if (address == this.players[i]) {
                    if (!serverAddress) this.sendState()
                    return i
                }
        ...
        private processControlServer(address: string, data: Buffer): void {
            ...
            if (!device)
                // can't find any device at that address
                return

            // check if prompted already
            if (this.promptedServers.indexOf(device.deviceId) >= 0) return
        ...
        private askJoin(device: Device): boolean {
            ...
            // cache user answer
            if (answer !== null) this.promptedServers.push(device.deviceId)
        ...
        private processClientButtons(address: string, data: Buffer): void {
            ...
            const player = controller
                .players()
                .find(p => p.playerIndex == playerIndex)
            if (!player) {
        ...
        sendState() {
            ...
            for (let p of this.players) {
                if (p) d.write(i, Buffer.fromHex(p))
                i += 8
            }
    ...
    function joinGame() {
        ...
        game.onPaint(() => {
            ...
            else game.showDialog(`disconnected`, `connect jacdac`)
        })
