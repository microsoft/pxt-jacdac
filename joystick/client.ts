namespace modules {
    ...
    export class JoystickClient extends jacdac.SensorClient {
        private readonly _variant: jacdac.RegisterClient<
            [jacdac.JoystickVariant]
        >
        private readonly _buttonsAvailable: jacdac.RegisterClient<
            [jacdac.JoystickButtons]
        >

        constructor(role: string) {
            super(jacdac.SRV_JOYSTICK, role, "u32 i1.15 i1.15")

            this._variant = this.addRegister<[jacdac.JoystickVariant]>(
                jacdac.JoystickReg.Variant,
                "u8"
            )
            this._buttonsAvailable = this.addRegister<[jacdac.JoystickButtons]>(
                jacdac.JoystickReg.ButtonsAvailable,
                "u32"
            )
        }
        ...
        //% weight=100
        isButtonDown(button: jacdac.JoystickButtons): boolean {
