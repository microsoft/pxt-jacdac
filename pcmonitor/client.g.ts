namespace modules {
    ...
    export class PCMonitorClient extends jacdac.Client {
        ...
        private readonly _gpuInformation: jacdac.RegisterClient<
            [number, number]
        >
        private readonly _networkInformation: jacdac.RegisterClient<
            [number, number]
        >
        ...
        constructor(role: string) {
            ...
            this._cpuUsage = this.addRegister<[number]>(
                jacdac.PCMonitorReg.CpuUsage,
                jacdac.PCMonitorRegPack.CpuUsage
            )
            this._cpuTemperature = this.addRegister<[number]>(
                jacdac.PCMonitorReg.CpuTemperature,
                jacdac.PCMonitorRegPack.CpuTemperature
            )
            this._ramUsage = this.addRegister<[number]>(
                jacdac.PCMonitorReg.RamUsage,
                jacdac.PCMonitorRegPack.RamUsage
            )
            this._gpuInformation = this.addRegister<[number, number]>(
                jacdac.PCMonitorReg.GpuInformation,
                jacdac.PCMonitorRegPack.GpuInformation
            )
            this._networkInformation = this.addRegister<[number, number]>(
                jacdac.PCMonitorReg.NetworkInformation,
                jacdac.PCMonitorRegPack.NetworkInformation
            )
        }
